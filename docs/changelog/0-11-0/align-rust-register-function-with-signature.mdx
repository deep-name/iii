...
If `.request_format` or `.response_format` is set on the builder, that schema wins over any auto-extracted one. With `RegisterFunction::untyped` and `RegisterFunction::http` no auto-extraction happens at all, so these setters are the only way to attach a schema.

## Removed

| Removed | Replacement |
| --- | --- |
| `III::register_function_with(id, handler, options)` | `III::register_function(id, RegisterFunction::untyped(handler).description(...))` (or the appropriate constructor) |
| `register_function((RegisterFunctionMessage, handler))` tuple form | `register_function(id, RegisterFunction::untyped(handler))` |
...
2. With a single entry point and a single registration type, the surface area becomes one method and one builder. Auto-schema extraction is preserved for typed functions; an explicit `untyped` path covers untyped `Value` closures; `http` covers HTTP-invoked functions.
...
| `register_function((RegisterFunctionMessage::with_id("id".into()), handler))` | `register_function("id", RegisterFunction::untyped(handler))` |
...
| `register_function_with("id", handler, RegisterFunctionOptions { description: Some("d".into()), ..Default::default() })` | `register_function("id", RegisterFunction::untyped(handler).description("d"))` |
