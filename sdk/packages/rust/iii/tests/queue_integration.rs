...
async fn void_returns_null_immediately() {
    ...
    iii.register_function(
        "test::queue::void::rs",
        RegisterFunction::raw(move |_input: Value| {
...
async fn durable_subscriber_receives_published_message() {
    ...
    let fn_ref = iii.register_function(
        function_id.clone(),
        RegisterFunction::raw(move |data: Value| {
            let received = received_clone.clone();
            async move {
...
async fn durable_subscriber_condition_function_filters_messages() {
    ...
    let fn_ref = iii.register_function(
        function_id.clone(),
        RegisterFunction::raw(move |_data: Value| {
    ...
    let cond_fn = iii.register_function(
        ...
        RegisterFunction::raw(move |input: Value| async move {
            let accept = input
                .get("accept")
