...
// export a non-empty service.name resource through the `both` exporter path.
//
...
#[tokio::test]
async fn both_exporter_forwards_engine_resource_to_otlp_collector() {
    ...
    let config = iii::workers::observability::otel::OtelConfig {
        ...
        service_version: "both-test-version".to_string(),
        service_namespace: None,
        exporter: iii::workers::observability::otel::ExporterType::Both,
        endpoint,
        sampling_ratio: 1.0,
        memory_max_spans: 10,
    };
    ...
    tracing::subscriber::with_default(subscriber, || {
        let span = tracing::info_span!("engine-originated-both-test-span");
        let _enter = span.enter();
        tracing::info!("emitting engine-originated span through the both exporter");
    });
    ...
    let resource = resource_for_span(&captured[0], "engine-originated-both-test-span")
    ...
    assert_eq!(
        ...
        Some("iii-engine"),
        "the both exporter must forward the provider Resource to its inner OTLP exporter"
    );
    assert_eq!(
        find_string_attr(&resource.attributes, "service.version"),
        Some("both-test-version"),
        "the both exporter should preserve the configured service.version resource attribute"
    );
}
